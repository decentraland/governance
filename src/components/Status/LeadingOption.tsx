...
export default React.memo(function LeadingOption({
  status,
  leadingOption,
  metVP,
}: LeadingOptionProps) {
      ...
      {status !== ProposalStatus.Pending && (
        <Paragraph small secondary>
          {status === ProposalStatus.Active && (
            <span>
              {l("page.proposal_detail.leading_option_label") + ": "}
              <strong>{leadingOption}</strong>
            </span>
          )}
          {proposalFinished && (
            <span>
              {l("page.proposal_detail.finished_result_label") + ": "}
              {!metVP ? (
                l("page.proposal_detail.threshold_not_met_label")
              ) : (
                <strong>{leadingOption}</strong>
              )}
            </span>
