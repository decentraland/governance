import { DiscourseTopic } from '../../api/Discourse'
import { ProposalComment } from '../Proposal/types'
import { filterComments, DISCOURSE_USER } from './utils'
...
describe('filterUserComments', () => {
  it('should return a parsed list of the user comments with avatar, username, user comment, and comment date', () => {
    const filteredComments: ProposalComment[] = filterComments(discourseTopic)
    expect(filteredComments[0].username).toBe('yemel')
    expect(filteredComments[0].avatar_template).toBe('/user_avatar/forum.decentraland.vote/yemel/{size}/1_2.png')
  ...
  describe('when there are DAO/system comments and user comments on a discourse topic', () => {
    it('should only retrieve the user comment ', function() {
      const filteredComments: ProposalComment[] = filterComments(discourseTopic)
