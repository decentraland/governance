...
export async function updateProposalStatus(req: WithAuth<Request<{ proposal: string }>>) {
  ...
  } else if (update.status === ProposalStatus.Prioritized) {
    update.prioritized_by = user;
    update.prioritized_description = configuration.description || null;
