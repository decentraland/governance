...
import { Badge, BadgeCreationResult, RevokeOrReinstateResult, UserBadges } from '../entities/Badges/types'
...
export class Governance extends API {
  ...
  async getCoreUnitsBadges() {
    const response = await this.fetch<ApiResponse<Record<string, Badge[]>>>(`/badges/core-units`)
    return response.data
  }
