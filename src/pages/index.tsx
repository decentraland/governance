...
export default function IndexPage() {
  ...
  const [ proposals, proposalsState ] = useProposals({ type, status, page, search, timeFrame, order, itemsPerPage: ITEMS_PER_PAGE })
          ...
          >
            ...
            >
              ...
              <div className="OnlyDesktop">
                {!searching && <SubscriptionBanner active={!type} />}
              </div>
              {type && !searching && <CategoryBanner type={type} active />}
              {proposals && proposals.data.length === 0 && <Empty
                description={(searching || status || timeFrame?.length > 0)? l(`navigation.search.no_matches`) : l(`page.proposal_list.no_proposals_yet`)}
              />}
