...
type DraftState = {
  linked_pre_proposal_id: string | null,
  title: string,
  summary: string,
  abstract: string,
  motivation: string,
...
const validate = createValidator<DraftState>({
  ...
  '*': (state) => ({
    linked_pre_proposal_id: (
      assert(!!state.linked_pre_proposal_id, 'error.draft.linked_proposal_empty')
    ),
    title: (
...
export default function SubmitDraftProposal() {
  ...
  const [passedProposals] = useAsyncMemo(async () => Governance.get().getPassedProposals(ProposalType.Poll), [])
      ...
      Promise.resolve()
        .then(async () => {
          return Governance.get().createProposalDraft({
            ...state.value,
            linked_pre_proposal_id: state.value.linked_pre_proposal_id!
          })
        })
        .then((proposal) => {
  ...
  return <ContentLayout small>
    ...
    <ContentSection>
      ...
      <SelectField
        value={state.value.linked_pre_proposal_id || undefined}
        placeholder={l('page.submit_draft.linked_proposal_placeholder') || undefined}
        onChange={(_, { value }) => editor.set({ linked_pre_proposal_id: String(value) })}
        options={passedProposals || undefined}
        error={!!state.error.linked_pre_proposal_id}
        message={l.optional(state.error.linked_pre_proposal_id)}
      />
    </ContentSection>
