...
export default function SubmitPOI() {
    ...
    if (state.validated) {
      Promise.resolve()
        .then(async () => {
          ...
          if (!(await isAlreadyPointOfInterest(x, y))) {
            throw new Error(`error.poi.coordinates_are_not_a_poi`)
          }

          return Governance.get().createRemoveProposalPOI({
            x,
            y,
  ...
  if (!account) {
    return (
      <Container>
        <Head
          title={l("page.submit_poi.remove.title") || ""}
  ...
  return (
    <ContentLayout small>
      <Head
        ...
        image="https://decentraland.org/images/decentraland.png"
      />
      <Helmet title={l("page.submit_poi.remove.title") || ""} />
      <ContentSection>
        <Header size="huge">{l("page.submit_poi.remove.title")}</Header>
      </ContentSection>
      <ContentSection>
        <Paragraph small>{l("page.submit_poi.description")}</Paragraph>
      </ContentSection>
      <ContentSection className="CoordinatesField">
        <Label>{l("page.submit_poi.remove.coordinates_label")}</Label>
      ...
      <ContentSection>
        ...
        <Paragraph tiny secondary className="details">
          {l("page.submit_poi.remove.description_detail")}
        </Paragraph>
        <MarkdownTextarea
          minHeight={175}
          value={state.value.description}
          placeholder={l("page.submit_poi.remove.description_placeholder")}
